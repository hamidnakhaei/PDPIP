...
class TabuSearchSolver(VRPSolverBase):
    ...
    def __init__(self, data: Dict):
        ...
        # Paper Section 5.2: "at most 1000 stagnant iterations"
        from constants import MAX_STAGNANT_ITERATIONS
        self.tabu_tenure_base = 3
        self.max_iterations_base = max(2000, MAX_STAGNANT_ITERATIONS + 500)
        self.max_non_improving_base = MAX_STAGNANT_ITERATIONS
    ...
    def solve_instance(self, instance_idx: int, num_realizations: int = 3) -> Dict:
        ...
        try:
            ...
            # Apply enhanced Tabu Search (with time limit to avoid runaway on large instances)
            from constants import TABU_TIME_LIMIT_SECONDS
            best_routes = self._enhanced_tabu_search(
                current_routes, instance_idx,
                start_time=start_time, time_limit_seconds=TABU_TIME_LIMIT_SECONDS,
            )
    ...
    def _adjust_parameters(self, problem_size: int):
        """Set parameters. Paper: 1000 stagnant iterations; iteration cap allows that."""
    ...
    def _enhanced_tabu_search(
        ...
        """Enhanced Tabu Search with feasibility focus. Stops when time_limit_seconds is reached if set."""
