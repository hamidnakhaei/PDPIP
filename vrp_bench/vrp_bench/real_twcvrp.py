...
from constants import (
    ...
    DEMAND_RANGE,
    MAP_SIZE,
    REALIZATIONS_PER_MAP,
)

# Paper: "60% residential, 40% commercial"
RESIDENTIAL_PROB = 0.6


def generate_time_window(use_paper_ratio: bool = False) -> Tuple[int, int]:
    if use_paper_ratio:
        customer_type = int(np.random.choice([0, 1], p=[RESIDENTIAL_PROB, 1 - RESIDENTIAL_PROB]))
    else:
        customer_type = random.randint(0, 1)
    return sample_time_window(customer_type)
...
def generate_twcvrp_instance(
    ...
    use_paper_time_ratio: bool = False,
) -> Dict:
    ...
    for index in tqdm(
        ...
        if index % REALIZATIONS_PER_MAP == 0:
            instance = generate_twcvrp_instance(
                ...
                use_paper_demand=use_paper_demand,
