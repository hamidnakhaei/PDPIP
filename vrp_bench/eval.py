...
def main(tsptw_run=None, sizes=None):
    ...
    eval_results_dir = os.path.join(os.path.dirname(__file__), "eval_results", run_timestamp)
    os.makedirs(eval_results_dir, exist_ok=True)

    # Initialize evaluator (TSP-TW only)
    evaluator = VRPEvaluator(
        base_path=config["base_path"],
        problem_set="tsptw",
        results_dir=eval_results_dir,
        tsp_tw_run=tsptw_run,
    )
    ...
    for solver_class, name in solvers:
        ...
        try:
            results[name] = evaluator.evaluate_solver(
                ...
                use_paper_protocol=config.get('use_paper_protocol', True),
            )
    ...
    # Generate ALL LaTeX tables (write to file for viewing)
    print(f"\n{'='*80}")
...
def generate_comparative_analysis_tables(results: Dict):
    ...
    # Table: Constraint violation breakdown (TSP-TW only: time window CVR = total CVR)
    ...
    for solver_name, result in results.items():
        if result and 'overall' in result:
            ...
            tw_viol = tsp_tw.get('time_window_violations', 0)
            print(f"{solver_name} & {total_cvr:.1f} & {feasibility:.3f} & {tw_viol:.2f} \\\\")
