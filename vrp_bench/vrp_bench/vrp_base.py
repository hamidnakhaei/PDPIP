...
from travel_time_generator import sample_travel_time, deterministic_travel_time, get_distances
...
class VRPSolverBase(ABC):
    ...
    def __init__(self, data: Dict):
        ...
        TSP mode: omit demands, num_vehicles, vehicle_capacities for single-vehicle no-capacity (depot 0, rest customers).
        """
        self.locations = self._convert_inhomogeneous_array(data['locations'])
        n_inst = self._n_instances_from_locations()
        n_nodes = self._n_nodes_from_locations()
        # TSP / no-capacity defaults when keys missing
        if 'demands' not in data or data['demands'] is None:
            self.demands = [np.array([0.0] + [1.0] * (n_nodes - 1), dtype=np.float64) for _ in range(n_inst)]
        else:
            self.demands = self._convert_inhomogeneous_array(data['demands'])
        if 'num_vehicles' not in data or data['num_vehicles'] is None:
            self.num_vehicles = [1] * n_inst
        else:
            self.num_vehicles = self._convert_inhomogeneous_array(data['num_vehicles'])
        if 'vehicle_capacities' not in data or data['vehicle_capacities'] is None:
            self.vehicle_capacities = [[1e9]] * n_inst
        else:
    ...
    def _n_instances_from_locations(self) -> int:
        """Number of instances from locations only (for TSP default construction)."""
        try:
            if isinstance(self.locations, list):
                return len(self.locations)
            if hasattr(self.locations, 'shape'):
    ...
    def _n_nodes_from_locations(self) -> int:
        """Number of nodes per instance from locations only (for TSP default construction)."""
        try:
            if isinstance(self.locations, list) and len(self.locations) > 0:
                loc0 = self.locations[0]
                if isinstance(loc0, np.ndarray):
                    return loc0.shape[0] if len(loc0.shape) >= 2 else (len(loc0) // 2)
                if hasattr(loc0, '__len__'):
                    return len(loc0) // 2
            if hasattr(self.locations, 'shape'):
    ...
    def _check_feasibility(self, routes: List[List[int]], instance_idx: int) -> Tuple[float, float, int]:
        ...
        for route_idx, route in enumerate(routes):
            ...
            # Get vehicle capacity (TSP: skip capacity check when no demand or capacity is effectively infinite)
            vehicle_capacity = capacities[route_idx] if route_idx < len(capacities) else (capacities[0] if len(capacities) > 0 else 100)
            route_demand = 0
            current_time = 0
            no_capacity_constraint = np.sum(demands) == 0 or vehicle_capacity >= 1e8
            ...
            # Check capacity violation (skip for TSP / no-capacity)
            if not no_capacity_constraint and route_demand > vehicle_capacity * 1.001:  # Small tolerance for floating point
    ...
    def _calculate_stochastic_cost(self, routes: List[List[int]], instance_idx: int, 
        ...
        # Robustness per paper Eq.18: ROB = (1/N) * sum_i (TC_i - mean_TC)^2 (variance)
        if len(total_costs) > 1:
            mean_cost = avg_cost
            robustness = sum((cost - mean_cost) ** 2 for cost in total_costs) / len(total_costs)
        else:
            robustness = 0.0
    ...
    def _simulate_route_execution(self, route: List[int], instance_idx: int) -> Tuple[float, float]:
        ...
        for i in range(len(route) - 1):
            ...
            next_node = route[i + 1]
            # Optional: use deterministic travel time for the first segment only
            if self.use_deterministic_first_segment and i == 0:
                travel_time = deterministic_travel_time(current_node, next_node, distance_dict)
            else:
