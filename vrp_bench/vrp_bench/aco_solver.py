...
class ACOSolver(VRPSolverBase):
    ...
    def __init__(self, data: Dict):
        ...
        self.num_ants = 50  # Paper Appendix B: m=50 ants
        self.max_iterations = max(MAX_STAGNANT_ITERATIONS, 1000)
        self.max_stagnant = MAX_STAGNANT_ITERATIONS
        self.alpha = 1.0  # Pheromone importance (Appendix B: α=1)
        self.beta = 2.0   # Heuristic importance (Appendix B: β=2)
        self.rho = 0.5    # Evaporation rate (Appendix B: ρ=0.5)
    ...
    def solve_instance(self, instance_idx: int, num_realizations: int = 3) -> Dict:
        ...
        try:
            ...
            # ACO main loop (paper: stop after 1000 stagnant iterations)
            for iteration in range(self.max_iterations):
                ...
                if iterations_without_improvement >= self.max_stagnant:
                    break
                if self.debug:
